...
def load_data2(dataset='mnist.pkl.gz'): 
    ''' Loads the dataset 
...
def sgd_optimization_mnist2(learning_rate=0.13, n_epochs=1000,dataset='mnist.pkl.gz',batch_size=600):
    """
...
class LogisticRegression3(object):
    """Multi-class Logistic Regression Class
    ...
    def __init__(self, input, n_in, n_out):
        ...
        # Returns a (samples*n_out) matrix with normalized values
    ...
    def errors(self, y):
        ...
        # else:
        #     raise NotImplementedError()
        return T.mean(y*(1-self.p_y_given_x) + (1-y)*(self.p_y_given_x))
        

    def printy(self):
        return self.y_pred

def load_data3(dataset):
    ''' Loads the dataset
...
def sgd_optimization_mnist3(learning_rate=0.13, n_epochs=1000,dataset='mnist.pkl.gz',batch_size=600):
    """
    Demonstrate stochastic gradient descent optimization of a log-linear
    model
    ...
    printy = theano.function(
        inputs=[index],
        outputs=classifier.printy(),
        updates=updates,
        givens={
            x: train_set_x[index * batch_size: (index + 1) * batch_size],
            y: train_set_y[index * batch_size: (index + 1) * batch_size]
...
def load_data(dataset):
    ...
    def shared_dataset(data_xy, borrow=True):
        # load data_x, data_y and rearange data_y in [00001000],[00100000]...
        data_x, data_y = data_xy
        b = numpy.zeros( (len(data_y),10) )
        b[numpy.arange(len(data_y)),data_y] = 1
...
def sgd_optimization_mnistOrig(learning_rate=0.13, n_epochs=1000,dataset='mnist.pkl.gz',batch_size=600):
    """
    Demonstrate stochastic gradient descent optimization of a log-linear
    model
    ...
    """
    datasets = load_dataOrig(dataset)
    ...
    # construct the logistic regression class
    # Each MNIST image has size 28*28
    classifier = LogisticRegressionOrig(input=x, n_in=28 * 28, n_out=10)
